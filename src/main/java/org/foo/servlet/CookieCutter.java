 ...
 * {@link #addCookieField(String)} method and parsed on the next subsequent call
 * to {@link #getCookies()}. If the added fields are identical to those last
 * added (as strings), then the cookies are not re parsed.
 */
					...
					if (invalue) {
						...
						case ';': {
							...
							if (quoted) {
								...
								value = tokenend >= tokenstart ? hdr.substring(tokenstart, tokenend + 1)
						...
						case '=':
							if (quoted) {
								...
								name = tokenend >= tokenstart ? hdr.substring(tokenstart, tokenend + 1)
										: hdr.substring(tokenstart);
